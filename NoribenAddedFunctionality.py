...
global_whitelist = [r'VMwareUser.exe',  # VMware User Tools
                    ...
                    #own:
                    r'python.exe', # this process will always launch Procmon
                    r'procmon.exe', # Procmon
                    r'cmd.exe', # although this is often involved, it should only be relevant if called by malware.
                    r'svchost.exe', # Loads a shell to host DLL services.
                    r'WindowsAzureGuestAgent.exe', # manages VM interaction with the Azure Fabric Controller
                    r'sihost.exe', # Shell Infrastructure Host handles graphical elements of the OS interface
                    r'RuntimeBroker.exe', # permission manager, checks application permissions
                    r'ApplicationFrameHost.exe', # designed to display certain applications in frames
                    r'Explorer.EXE', # manages the graphical user interface. Without it, the interface will disappear.
                    r'ctfmon.exe', # activates the language bar in Microsoft Office
                    r'MonitoringHost.exe', # health serviced process, belongs to System Center Operations Manager
                    r'cscript.exe', # allows users to run scripts by typing its name at the command line prompt
                    r'MMAExtensionHeartbeatService.exe', # Microsoft Monitoring Agent Azure VM Extension Heartbeat Service
                    r'MsMpEng.exe', # performs malware checks for Windows Defender (which will be turned off for testing)
                    r'WindowsAzureTelemetryService.exe', # automated process to send metrics and other data to Azure
                    r'taskhostw.exe', # part of Windows 10, starts DLL-based Windows services
                    r'GoogleUpdate.exe', # downloads, installs, removes and automatically updates Google applications
                    r'NetworkWatcherAgent.exe', # Azure network performance monitoring, diagnostic, and analytics service
                    r'HealthService.exe', # monitors performance
                    r'WaAppAgent.exe', # process for configuring, managing, and accelerating Virtual Machines
                    r'WaSecAgentProv.exe', # Windows Azure Security Agent Provider
                    r'VFPlugin.exe', # standard Windows process 
                    r'sppsvc.exe', # Microsoft Software Platform Protection Service – handles Microsoft licenses
                    r'TrustedInstaller.exe', # enables installation, modification, and removal of Windows updates
                    r'TiWorker.exe', # child process of TrustedInstaller.exe
                    r'dwm.exe', # runs the Desktop Windows Manager
                    r'ShellExperienceHost.exe', # helps built-in universal apps integrate seamlessly with the Windows Shell
                    r'csrss.exe', # Client Server Runtime Process to create/delete threads
                    r'Conhost.exe', # required to run in order for Command Prompt to interface with Windows Explorer
                    r'LogonUI.exe', # graphical user interface for user authentication
                    r'lsass.exe', # calls DLL's involved in critical Windows Security services
                    r'smartscreen.exe', # Windows service to check downloaded files. Comes up whenever I run a sample.
                    r'CHXSmartScreen.exe', # process belonging to smartscreen
                    r'DLLHost.exe', # launch Windows operating services or applications
                    r'sc.exe', # allows Windows to start, stop and interact with other processes.
                    r'fontdrvhost.exe', # manages font drivers
                    r'services.exe', # launches Service Control Manager to start and stop Windows system processes
                    r'BackgroundTransferHost.exe', # Microsoft background transfer host utility
                    r'SearchFilterHost.exe', # component of Windows Search Indexer
                    r'pacjsworker.exe', # HTTP Auto Proxy Detection Worker Process
                    r'MpCmdRun.exe', # Microsoft Malware Protection Command-Run Utility
                    r'WindowsInternal.ComposableShell.Experiences.TextInput.InputApp.exe', # on screen keyboard
                    r'IaaSBcdrExtension.exe', # Microsoft Azure Backup for Windows Server
                    r'JsonVMAccessExtension.exe', # belongs to Azure VM access agent
                    r'MMAExtensionEnable.exe', # belongs to Microsoft monitoring agent
                    r'where.exe', # carry out file searches
                    r'TraceLogSM.exe', # belongs to Microsoft System Center Operations Manager
                    r'CollectGuestLogs.exe', # belongs to Azure, collects security logs
                    r'MOMPerfSnapshotHelper.exe', # component of the software System Center Operations Manager
                    r'SearchUI.exe', # component of Cortana
                    r'software_reporter_tool.exe', # Google Chrome Software Reporter Tool
                    r'WMIADAP.EXE', # associated with the AutoDiscovery/AutoPurge (ADAP) process of WMI (Windows Management Instrumentation)
                    r'mscorsvw.exe', # used to precompile Microsoft .NET framework assemblies in the background
                    r'MusNotification.exe', # related to displaying available Windows updates
                    r'UpdateNotificationMgr.exe', # related to Windows updates
                    r'devicecensus.exe', # belongs to Telemetry service, gathers data on webcam usage
                    r'MusNotificationUx.exe', # related to displaying available Windows updates
                    r'wsqmcons.exe', # related to collecting and sending usage data to Microsoft
                    r'usoclient.exe', # Windows 10 update orchestrator
                    r'WerFault.exe', # Windows error reporting
                    r'smss.exe', # manages the startup of all user sessions in Windows
                    r'SgrmBroker.exe', # System Guard Runtime Monitor Broker
                    r'CompatTelRunner.exe', # scans application files to assess their compatibility with Windows 10 if an upgrade is performed
                    r'compattelrunner.exe', #  assists the above
                    r'wermgr.exe', # collect and send post-error debugging information via the Internet to Microsoft
                    r'WindowsAzureNetAgent.exe', # default process belonging to Azure running Windows 10 Pro
                    r'HxTsr.exe', # establishes and maintains contact between the Outlook app and Outlook.com
                    r'vssvc.exe', # Shadow Copy is a Windows backup utility that allows backups of snapshots even when machine is in use
                    r'IaaSBcdrExtension.exe', # Windows Azure Backup Integration Module Service
                    r'SecurityHealthHost.exe', # Microsoft
                    '^System$', # processes that are called System, not that contain the word somewhere.
                    '^Registry$', # processes that are called Registry. Registry:88 is a standard process.
                    r'usocoreworker.exe', # Update Session Orchestrator which replaced Windows Update Agent.
                    r'StartMenuExperienceHost.exe', # New process to separate Start Menu from Explorer process.
                    r'backgroundTaskHost.exe',
                    r'WindowsAzureGuestAgent.exe',
                    r'WmiApSrv.exe' ,
                    r'dismhost.exe', # Deployment Image Service Managing Host
                    r'wmiprvse.exe',
                    r'ielowutil.exe',
                    r'IEXPLORE.EXE',
                    r'CompPkgSrv.exe'					
...
file_whitelist = [r'Desired Access: Execute/Traverse', # comment these out to see more
...
reg_whitelist = [r'CaptureProcessMonitor',
                 ...
                 #own:
                 r'HKLM\System\CurrentControlSet\Services\bam\State\UserSettings\S-1-5-21-2259584852-2365352558-1469986205-500\SequenceNumber',
                 r'HKLM\System\CurrentControlSet\Services\W32Time\SecureTimeLimits',
                 # Multiple events with this come up in Calculator, setting a key to some hex value.
                 r'HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Notifications\Data',
                 # This one comes up every time cmd.exe runs a command. It is not informative about the command that is run, is just set to some hex value.
                 r'HKLM\System\CurrentControlSet\Services\bam\State\UserSettings\S-1-5-21-2259584852-2365352558-1469986205-500\\Device\HarddiskVolume2\Windows\System32'
                 ]

net_whitelist = [r'haspmls.exe'  # Hasp dongle beacons
...
def whitelist_scan(whitelist, data):
    ...
    """
    for event in data: #for field in fields:
        for bad in whitelist + global_whitelist: #for item in whitelist
            bad = os.path.expandvars(bad).replace('\\', '\\\\')
            try:
                if re.search(bad, event, flags=re.IGNORECASE): #if whitelist item is in field
                    return True
            except re.error:
...
# initialise empty variables here and set to global just before they are set to a value, so all the sections thereafter will be able to use it.
child_pid = None
bad_pid_list = []
tb_matched_pattern_list = []
def parse_csv(csv_file, report, timeline):
    ...
    # variables for standard powershell operations to skip:
    ps_skip1 = r'\AppData\Local\Temp\__PSScriptPolicyTest_'
    ps_skip2 = r'\AppData\Local\Microsoft\Windows\PowerShell\StartupProfileData-NonInteractive'
    ps_skip3 = r'C:\Windows\System32\CatRoot'
    ps_skip4 = r'C:\Windows\System32\catroot2'
    ps_skip5 = r'\AppData\Local\Microsoft\CLR_v4.0\UsageLogs\powershell.exe.log'
    ps_skip6 = r'\AppData\Local\Microsoft\Windows\PowerShell\ModuleAnalysisCache'
    ps_skip = [ps_skip1, ps_skip2, ps_skip3, ps_skip4, ps_skip5, ps_skip6]
    # prev_line is needed to remove subsequet duplicates from csv file.
    prev_line = ''
    # set to global so the subsequent sections can use the variables:
    global child_pid
    global bad_pid_list
    for original_line in reader:
        ...
        # From here ("try:"), the csv file is filtered for the final report.
        try:
            # At the start of each event scan, make bad pid list unique because it gets longer appended to with every scanned event
            bad_pid_list = list(set(bad_pid_list)) 
            # skip standard powershell operations
            if (field[1] == r'powershell.exe'):
                for skip_item in ps_skip:
                    if skip_item in field[4]:                
                        continue
                continue
            # If previous line in csv is same as current line, ignore and set current to previous line.
            if (field[1:4] == prev_line):
                prev_line = field[1:4]
                continue
            prev_line = field[1:4]
            # Now try to show all processes created by non-whitelisted processes, even if they're on the whitelist.
            if field[3] in ['Process Create'] and field[5] == 'SUCCESS':
                cmdline = field[6].split('Command line: ')[1]
                # If originating process is bad, or process came from a bad process, or good process created a bad process:
                if (not whitelist_scan(cmd_whitelist, field[1:2])) or (not whitelist_scan(cmd_whitelist, field[4:5])) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    if config['generalize_paths']:
                        cmdline = generalize_var(cmdline)
                    # Extract the child_pid from the event:
                    ...
                    # Add child PID to bad_pid_list, but don't do so if a process launches a command.
                    # For example, if a cmd.exe command is launched using /c option, a cmd.exe child process is created.
                    # That process would then just repeat the command already shown in the original system call.
                    if " /c " not in field[4]:
                        bad_pid_list.append(child_pid)
                    process_output.append(outputtext)
                    timeline.append(tl_text)
                    # If the originating process is bad, add it to bad_pid_list:
                    if (not whitelist_scan(cmd_whitelist, field[1:2])) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                        bad_pid_list.append(field[2])

            elif field[3] == 'WriteFile' and field[5] == 'SUCCESS':
                # add check to include operations by child processes that were created by non-whitelisted processes
                if (not whitelist_scan(file_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in other events
                    path = field[4]
                    yara_hits = ''
                    ...
                    if os.path.isdir(path):
                        ...
                        outputtext = '[WriteFolder] {}:{} > {}'.format(field[1], field[2], path)
                        tl_text = '{},File,WriteFolder,{},{},{}'.format(date_stamp, field[1],
                                                                         field[2], path)
                        ...
                        try:
                            ...
                            if config['generalize_paths']:
                                path = generalize_var(path)
                            outputtext = '[WriteFile] {}:{} > {}\t[{}: {}]{}{}'.format(field[1], field[2], path,
                                                                                        config['hash_type'], hashval,
                                                                                        yara_hits, av_hits)
                            tl_text = '{},File,WriteFile,{},{},{},{},{},{},{}'.format(date_stamp,
                            ...
                            if config['generalize_paths']:
                                path = generalize_var(path)
                            outputtext = '[WriteFile] {}:{} > {}\t[File no longer exists]'.format(field[1], field[2],
                                                                                                   path)
                            tl_text = '{},File,WriteFile,{},{},{},N/A'.format(date_stamp,
                                                                               field[1], field[2], path)
                            file_output.append(outputtext)
                            timeline.append(tl_text)
                ...
                # add check to include operations by child processes that were created by non-whitelisted processes
                if (not whitelist_scan(file_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in the events
                    path = field[4]
                    if config['generalize_paths']:
                        path = generalize_var(path)
                ...
                # add check to include operations by child processes that were created by non-whitelisted processes but are also not on reg whitelist
                if (not (whitelist_scan(reg_whitelist, field))) or ((field[2] in bad_pid_list or field[7] in bad_pid_list) and (not (whitelist_scan(reg_whitelist, field)))):
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in the events
                ...
                # add check to include operations by child processes that were created by non-whitelisted processes
                if (not whitelist_scan(net_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in the events
            ...
            elif field[3] == 'TCP Reconnect' and field[5] == 'SUCCESS':
                # add check to include operations by child processes that were created by non-whitelisted processes
                if (not whitelist_scan(net_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in the events
                    server = field[4].split('-> ')[1]
                    outputtext = '[TCP] {}:{} > {}'.format(field[1], field[2], protocol_replace(server))
                    if outputtext not in net_output:
                        tl_text = '{},Network,TCP Reconnect,{},{},{}'.format(date_stamp, field[1],
                                                                        field[2], protocol_replace(server))
                        net_output.append(outputtext)
                        timeline.append(tl_text)
                        
            elif field[3] == 'TCP Disconnect' and field[5] == 'SUCCESS':
                # add check to include operations by child processes that were created by non-whitelisted processes
                if (not whitelist_scan(net_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                    bad_pid_list.append(field[2]) # append PID to bad_pid_list to check for PPID in the events
                    server = field[4].split('-> ')[1]
                    outputtext = '[TCP] {}:{} > {}'.format(field[1], field[2], protocol_replace(server))
                    if outputtext not in net_output:
                        tl_text = '{},Network,TCP Disconnect,{},{},{}'.format(date_stamp, field[1],
                                                                        field[2], protocol_replace(server))
                        net_output.append(outputtext)
                        timeline.append(tl_text)

            # Else if any non-whitelisted process exists that maybe only read files or registry keys instead of modifying them, include ppid in case it launches other processes!
            elif (not whitelist_scan(cmd_whitelist, field)) or field[2] in bad_pid_list or field[7] in bad_pid_list:
                bad_pid_list.append(field[2])   
    ...
    # remove subsequent duplicates from each section in Noriben report using prev_event, and the if-else statements in each section:
    prev_event = ''
    ...
    # Include result lines to say if sample is Trickbot and what patterns were matched.
    report.append('')
    report.append('Analysis:')
    report.append('==================')
    log_debug('[*] Writing %d Analysis results to report' % (len(analysis_output)))
    # Define tuples with descriptor and Regex for each Trickbot regex pattern:
    # process-related trickbot patterns:
    tb_proc_svchost = (r"Creates svchost.exe process", "^\[CreateProcess\] .* > \"\%WinDir\%\\\\system32\\\\svchost\.exe\"")
    tb_proc_windefend = (r"Launches command to delete or stop Windows Defender", "^\[CreateProcess\] .* > \".*sc\s+(delete|stop) WinDefend")
    tb_proc_mppreference = (r"Uses Powershell Set-MpPreference command to change antivirus preferences", "^\[CreateProcess\] .* > \".* \/c powershell Set-MpPreference")
    tb_proc_allusersprofile_exe = (r"Launches executable file in %AllUsersProfile% folder", "^\[CreateProcess\] .* > \"\%AllUsersProfile\%\\\\.*\.exe\"")
    tb_proc_appdata_exe = (r"Launches executable file in subfolder of %AppData% folder", "^\[CreateProcess\] .* > \"\%AppData\%\\\\.*\\\\.*\.exe\"")
    tb_proc_tmp1 = (r"Launches tmp1.exe process from %LocalAppData\Temp folder", "^\[CreateProcess\] .* > \"\%LocalAppData\%\\\\Temp\\\\\tmp1\.exe\"")
    # file-related trickbot patterns:
    tb_write_rsa = (r"Writes to file in ...\Microsoft\Crypto\RSA\<SID> folder", "^\[WriteFile\] .* > .*\\\\Microsoft\\\\Crypto\\\\RSA\\\\S-1-5-")
    tb_write_settings_ini = (r"Writes to settings.ini file", "^\[WriteFile\] .* > .*\\\\settings\.ini")
    tb_write_allusersprofile_exe = (r"Writes to executable file in %AllUsersProfile%", "^\[WriteFile\] .* > \%AllUsersProfile\%\\\\.*\.exe\t")
    tb_write_appdata_exe = (r"Writes to executable file in subfolder of %AppData% folder", "^\[WriteFile\] .* > \%AppData\%\\\\.*\\\\.*\.exe")
    tb_write_tmp1 = (r"Writes to file %LocalAppData%\Temp\tmp.exe", "^\[WriteFile\] .* > \%LocalAppData\%\\\\Temp\\\\tmp1\.exe")
    # registry_related trickbot patterns:
    tb_set_notifications = (r"Disables Windows Defender Security Center notifications in registry", "^\[RegSetValue\] .* > .*\\\\Windows Defender Security Center\\\\.*\\\\DisableNotifications  =  1")
    tb_set_windowsdefender = (r"Changes Windows Defender registry settings", "^\[RegSetValue\] .* > .*\\\\Windows Defender\\\\.*  =  1")
    tb_set_proxybypass = (r"Sets registry subkey in HKU\.DEFAULT to bypass proxy settings", "^\[RegSetValue\] .* > HKU\\\\\.DEFAULT\\\\.*\\\\ProxyBypass  =  1")
    ...
    tb_pattern_list = [tb_proc_svchost, tb_proc_windefend, tb_proc_mppreference, tb_proc_allusersprofile_exe, tb_proc_appdata_exe, tb_proc_tmp1, 
	  tb_write_rsa, tb_write_settings_ini, tb_write_allusersprofile_exe, tb_write_appdata_exe, 
	  tb_write_tmp1, tb_set_notifications, tb_set_windowsdefender, tb_set_proxybypass, tb_svchost_443, tb_svchost_449, tb_rundll_5900]
    # Initiate new list to be used for compiled regular expressions from raw_list:
    tb_reg_list = []
    # Access the second item in each tuple in the tb_pattern_list list of tuples, which is the raw uncompiled regex:
    # Make sure compiled regex are in iterable list form:
    for tb_raw in [x[1] for x in tb_pattern_list]:
	    tb_reg_list.append(re.compile(tb_raw))
    # tb_matched_pattern_list will be appended to with lists matching a regex, for output in report:
    global tb_matched_pattern_list
    # Match each regex to each line in report and write matches to match_list:
    for line in report:
        # Integer i will help to find matching tuple in tb_pattern_list after matching compiled regex from tb_reg_list:
        i=0
        for tb_reg in tb_reg_list:
            # If match is found, add the corresponding pattern descriptor (1st item of tuple) to tb_matched_pattern_list:
            if tb_reg.search(line):
                tb_matched_pattern_list.append(tb_pattern_list[i][0])
            i += 1
    # If patterns were found, append lines with counter and pattern descriptor to report:
    if len(tb_matched_pattern_list) > 0:   
        analysis = []    
        report.append(str(len(tb_matched_pattern_list)) + " Trickbot pattern(s) found:")
        # Append a line for each pattern found with a count of how often that pattern was found:
        for tb_pattern_descriptor in tb_matched_pattern_list:
            tb_match_count = tb_matched_pattern_list.count(tb_pattern_descriptor)
            analysis.append("(" + str(tb_match_count) + ") " + tb_pattern_descriptor)
        # Add unique analysis lines to report:
        for line in list(set(analysis)):
            report.append(line)
    # If no patterns were found, print message:
    elif len(tb_matched_pattern_list) == 0:
        report.append("No Trickbot patterns found.")
